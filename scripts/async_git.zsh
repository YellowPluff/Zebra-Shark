...
__run_gather_git_info()
{
    ...
    # Submodule
    if [[ -n $($GIT_EXEC rev-parse --show-superproject-working-tree 2> /dev/null) ]];
    then
    ...
    # Branch name
    if branch_name="$($GIT_EXEC symbolic-ref -q --short HEAD 2> /dev/null)";
    then
    ...
    # Merging / Rebasing
    # This is bugged. Need a different way to detect rebase / merge.
    # GIT_DIR="$($GIT_EXEC rev-parse --git-dir 2> /dev/null)"
    # if [[ -n "$GIT_DIR" ]];
    # then
    ...
    # Untracked files
    if [[ -n $($GIT_EXEC ls-files --other --directory --exclude-standard --no-empty-directory 2> /dev/null) ]];
    then
        untracked_files="-"
    fi

    # Modified (unstaged) files
    if ! $GIT_EXEC diff --quiet 2> /dev/null;
    then
        unstaged_changes="*"
    fi

    # Modified (staged) files
    if ! $GIT_EXEC diff --cached --quiet 2> /dev/null;
    then
        staged_changes="+"
    fi
...
__git_information_worker_completed()
{
...
{
    if [[ $($GIT_EXEC rev-parse --is-inside-work-tree 2> /dev/null) == 'true' ]];
    then
        # You have to reset GIT_INFO_MESSAGE here.
        # There are 3 ways to do it...
        # 1. Unset GIT_INFO_MESSAGE. This will leave the git prompt blank while it loads
        # 2. GIT_INFO_MESSAGE=... will show three 'loading' dots while it loads
        # 3. GIT_INFO_MESSAGE=$GIT_INFO_MESSAGE will show the last git information while the new git information loads.
        #       This could be misleading though in the event git doesn't load instantly.
...
# Used in the ctrl+u logic for defrawing prompt in default_functions.zsh
USING_ASYNC_GIT=1
...
source $installPath/scripts/external_plugins/zsh-async-1.8.6/async.zsh
